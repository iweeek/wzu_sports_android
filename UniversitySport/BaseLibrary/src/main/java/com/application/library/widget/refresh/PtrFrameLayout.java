...
import com.application.library.widget.refresh.util.PtrCLog;
 ...
 * It defines {@link com.application.library.widget.refresh.PtrUIHandler}, which allows you customize the UI easily.
 */
public class PtrFrameLayout extends ViewGroup {
    ...
    private FrameSearchHeader.OnSearchClickListener searchClickListener;
    ...
    public void setOnSearchClickListener(FrameSearchHeader.OnSearchClickListener clickListener) {
        this.searchClickListener = clickListener;
        if(getHeaderView() != null && getHeaderView() instanceof FrameSearchHeader){
            ((FrameSearchHeader) getHeaderView()).setOnSearchClickListener(searchClickListener);
        }
    }
    ...
    public boolean dispatchTouchEvent(MotionEvent e) {
        ...
        switch (action) {
            ...
            case MotionEvent.ACTION_MOVE:
                ...
                if (mDisableWhenHorizontalMove && !mPreventForHorizontal && (Math.abs(offsetY) < mPagingTouchSlop && Math.abs(offsetX) > Math.abs(offsetY))) {
...
//        if ((deltaY < 0 && mPtrIndicator.getCurrentPosY() == PtrIndicator.POS_START)) {
            if (DEBUG) {
                PtrCLog.e(LOG_TAG, String.format("has reached the top"));
            }
            if (mPtrIndicator.isHasSearchBar()) {
                mPtrIndicator.setShowSearchBar(false);
            }
            return;
        }
    ...
    private void tryScrollBackToTop() {
        if (!mPtrIndicator.isUnderTouch()) {
            if (mStatus != PTR_STATUS_COMPLETE && mPtrIndicator.isHasSearchBar() && (mPtrIndicator.getCurrentPosY() > mPtrIndicator.getSearchHeight() || mPtrIndicator.getCurrentPosY() > mPtrIndicator.getSearchHeight() * 0.5f)) {
                mScrollChecker.tryToScrollTo(mPtrIndicator.getSearchHeight(), mDurationToCloseHeader);
                mPtrIndicator.setShowSearchBar(true);
            } else {
    ...
    public void setHeaderView(View header) {
        ...
        mHeaderView = header;
        addView(header);
        if (header instanceof FrameSearchHeader) {
            mPtrIndicator.setHasSearchBar(true);
            mPtrIndicator.setSearchHeight(((FrameSearchHeader) header).getSearchHeight());
        }
    }
