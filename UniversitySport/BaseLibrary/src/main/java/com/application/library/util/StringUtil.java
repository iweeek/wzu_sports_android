...
public class StringUtil {
    ...
    public static boolean checkMobilePhoneNo(String phoneNo) {
        ...
        try {
            if (!Pattern.compile(ZHENGZE_PHONE).matcher(phoneNo).matches()) {
                result = false;
            } else {
                result = true;
            }
    ...
    public static String formatNumber(Context context,long number) {
        ...
        if (number >= 10000000) {
            ...
            numberStr = String.valueOf(decimalFormat.format(((double) number) / 10000000));
            return numberStr + context.getString(R.string.unit_qian_wan);
        } else if (number >= 10000) {
            DecimalFormat decimalFormat = new DecimalFormat("#.0");
            decimalFormat.setRoundingMode(RoundingMode.DOWN);
            numberStr = String.valueOf(decimalFormat.format(((double) number) / 10000));
            return numberStr + context.getString(R.string.unit_wan);
        } else {
            return numberStr;
        }
    }
    ...
    private static final String EMOJISTR = "[\ud83c\udc00-\ud83c\udfff]|[\ud83d\udc00-\ud83d\udfff]|[\u2600-\u27ff]";

    public static boolean checkEmojText(String content) {
        Pattern emoji = Pattern.compile(EMOJISTR, Pattern.UNICODE_CASE | Pattern.CASE_INSENSITIVE);
        Matcher matcher = emoji.matcher(content);
        return matcher.find();
    }

    public static String replaceEmojText(String content) {
        Pattern emoji = Pattern.compile(EMOJISTR, Pattern.UNICODE_CASE | Pattern.CASE_INSENSITIVE);
        Matcher matcher = emoji.matcher(content);
    ...
    public static String getMoney(int money) {
        return getMoney(Long.valueOf(money));
    }
    ...
    public static SpannableString get60TextColor(Context context, String string) {
...
//        AbsoluteSizeSpan absoluteSizeSpan = new AbsoluteSizeSpan(12, true);
        ForegroundColorSpan foregroundColorSpan = new ForegroundColorSpan(context.getResources().getColor(R.color.text_black_90));
        spannableString.setSpan(foregroundColorSpan, 1, sb.length() - 2, Spanned.SPAN_INCLUSIVE_INCLUSIVE);
//        spannableString.setSpan(absoluteSizeSpan, 1, sb.length() - 2, Spanned.SPAN_INCLUSIVE_INCLUSIVE);
     ...
     * @return
     */
    public static String formatPhoneNum(String formatNum) {
        try {
            Pattern p = Pattern.compile("(?<=\\d)(?=(\\d\\d\\d\\d)+$)");
            Matcher m = p.matcher(formatNum);
            return m.replaceAll(" ");
        } catch (Throwable e) {
            return formatNum;
        }
    ...
    public static void setupMatchTypeFace(Context context, TextView textView) {
        Typeface typeface = Typeface.createFromAsset(context.getAssets(), "font/match.ttf");
        if (typeface != null) {
